...
def main():
    ...
    for m in modalities:
        ...
        models[m] = getattr(model_list, args.models[m].model)(num_classes= num_classes)
    ...
    if args.action == "train":
      ...
      train_loader = torch.utils.data.DataLoader(SpectroDataset('../an_data/EMG_spec/ActionNet_EMG_train.pkl'), batch_size=args.batch_size, shuffle=True,
                                                   num_workers=args.dataset.workers, pin_memory=True, drop_last=True)

      val_loader = torch.utils.data.DataLoader(SpectroDataset('../an_data/EMG_spec/ActionNet_EMG_test.pkl'), batch_size=args.batch_size, shuffle=False,
                                                num_workers=args.dataset.workers, pin_memory=True, drop_last=False)        
         
      # LOADER GENERATED FOR SAVING EXTRACTED FEATURES
      loader = torch.utils.data.DataLoader(SpectroDataset('../an_data/EMG_spec/ActionNet_EMG_train.pkl'), batch_size=1, shuffle=True,
                                                   num_workers=args.dataset.workers, pin_memory=True, drop_last=True)

      loader_test = torch.utils.data.DataLoader(SpectroDataset('../an_data/EMG_spec/ActionNet_EMG_test.pkl'), batch_size=1, shuffle=True,
                                                   num_workers=args.dataset.workers, pin_memory=True, drop_last=True)
...
def save_feat(model, loader, device, it, num_classes, train=False, num_clips = 5, aug=None, **kwargs):
    ...
    with torch.no_grad():
        for _, (data, label, uid) in enumerate(loader):            
        ...
        if not os.path.isdir("../an_data/EMG_saved_features/"):
            os.makedirs("../an_data/EMG_saved_features/")
        filename = f"ActionNet_{'train' if train else 'test'}.pkl"
        pickle.dump(results_dict, open(os.path.join("../an_data/EMG_saved_features/",filename), 'wb'))
    return 0
